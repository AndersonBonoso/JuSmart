...
/** Google logo oficial (multicolor) */
const GoogleSVG = (props) => (
  <svg viewBox="0 0 48 48" width="18" height="18" aria-hidden="true" {...props}>
    ...
    <path fill="#FF3D00" d="M6.306,14.691l6.571,4.819C14.655,16.155,18.961,13,24,13c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657  C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"></path>
    <path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.197l-6.197-5.238C29.171,35.091,26.715,36,24,36 c-5.202,0-9.619-3.315-11.283-7.946l-6.522,5.024C9.508,39.556,16.227,44,24,44z"></path>
    <path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.236-2.231,4.166-4.091,5.565c0.001-0.001,0.002-0.001,0.003-0.002  l6.197,5.238C36.965,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
  </svg>
);

/** Apple glyph monocromático — herda a cor do texto */
  ...
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 ${className}`}>
      ...
      <Button
        ...
        className={`${heights[size]} bg-white text-slate-800 border border-slate-300 hover:bg-slate-50 shadow-sm`}
        aria-label={googleText}
      >
        <GoogleSVG className={`${iconSize} mr-2`} />
      ...
      <Button
        ...
        className={`${heights[size]} bg-black text-white hover:bg-black/90`}
        aria-label={appleText}
      >
        <AppleSVG className={`${iconSize} mr-2`} />
